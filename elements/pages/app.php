	...
	<div class="container">
	  <div class="crow">
	    <div class="ws-12">
				...
				<p>When including app.php it will use default parameters as seen above. You can override those by creating a config.json.php file. With App.php comes a dummy file for this, containing the basic structure for you overwrites. See SAMPLE.config.json.php</p>
				<p>
					<?
				...
				<p>If you decide to use a config.json.php file for you project, note that this config.json.php now have to be positioned in your project root to get the server variables accurate.</p>
				...
				<p>$app will now contain the basic server variables, but also those that you chosen to include, like in the example, mysql credentials.</p>
				...
				<p>$app->get works with it's own scope, so it's not like a regular php include call. If you want to pass any variable, arrays or other datas into the element you are rendering, simply pass them into the function like so:</p>
				<p><code>$app->get('pages/myphppage', array('variable' => $value, 'anothervariable' => 'staticvalue');</code></p>
				<p>This keeps your files clean from programming logics and only the variables passed can be used.</p>
				...
				<p>$app->includer(array()) scans the targeted directories from files/directories matching the inputs and includes those. This is very inconvenient instead of using phps "include" and/or "include_once" several times at the top of every page.</p>
				<p>Simply do:</p>
				<p><code>$app->includer(array('classes', 'misc/phptimefunctions', 'functions'));</code></p>
				<p>to include files.</p>
				<p>The code above will go through do (in order):</p>
				<ul>
				...
				<p>Like $app->get(), $app->includer() also forwards you $app object into all files included. That way you can use any system variables set in you config.json.php file.</p>
